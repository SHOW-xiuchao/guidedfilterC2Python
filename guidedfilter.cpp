...
// 解码这个结构到opencv cv:Mat 
cv::Mat decode_pyMat(pyMat a){
    uchar* data = new uchar[a.size];
    for (int i=0; i<a.size; i++){
        data[i] = a.data[i];
    }
    assert(a.channel == 3 || a.channel == 1);

    if (a.channel == 3){
        return cv::Mat(cv::Size(a.width,a.height),CV_8UC3, data);
    }
    return cv::Mat(cv::Size(a.width,a.height),CV_8UC1, data);
}
    

pyMat guidedFilter(pyMat I, pyMat p, int r, double eps, int depth){
    pyMat res;
    cv::Mat out = GuidedFilter(decode_pyMat(I), 
        r, eps).filter(decode_pyMat(p), depth);
    res.height = out.rows;
    res.width = out.cols;
    res.channel = out.channels();
    res.size = out.rows * out.cols * out.channels();
...
// 这个是你使用python会调用的函数
PyObject* PyguidedFilter(PyObject* self, PyObject* args){
    pyMat I,p;
    int r,depth;
    double eps;
    if (!PyArg_ParseTuple(args,"iiis#iiis#idi",&I.height,&I.width,
    &I.channel,&I.data,&I.size,&p.height,&p.width,&p.channel,&p.data,
    &p.size,&r,&eps,&depth)){
        return NULL;
    }
    pyMat res = guidedFilter(I,p,r,eps,depth);
    return Py_BuildValue("y#iii",res.data,res.size,res.height,res.width,res.channel);
}

// 注册的函数名字 以及对应的函数 
static PyMethodDef gf_methods[] = {
    {"guidedFilter", PyguidedFilter, METH_VARARGS},
    {NULL,NULL}
};
...
PyMODINIT_FUNC PyInit_guidedfilter(void){
